...
%Finite element solver for two dimensional poroelasticity
%Adilan W Mahdiyasa
%First version 2024
...
%-------------------------------------------------------------------------------
    input_data               = input_2D;     
    time_max                 = 10000;                       %Maximum simulation time 
...
% Assemble Global Stiffness KG and Damping Matrix CG 
    for iel=1:number_element;  
         [local_data]        = localize_2D(iel,input_data);
         Element_No          = iel;
 ...
 %        Assembling Global Stiffness and Global Damping matrix 
         [KG]               = assemble_2D (KG, Kl, local_data, totalelementdof); %Assemble global stiffness matrix KG from local stiffness matrix Kl
         [CG]               = assemble_2D (CG, Cl, local_data, totalelementdof); %Assemble global damping matrix GG from local damping matrix Cl
...
%         Assembling load and boundary conditions
        [F]                 = vectorstretch_2D (input_data.LOAD, number_nodes, 1+nodaldof);  %Creating vector column of load
        [con]               = vectorstretch_2D (input_data.CON,  number_nodes, 1+nodaldof) ;  %Creating vector column of boundary condition
        solution            = F.* (con); 
        F                   = F.* (1-con); 
        F                   = F - KG*solution; 
...
for i=1:number_nodes 
        displacement_hor(i)     =-solution((3*i)-2);     %Solution of horizontal displacement
        displacement_ver(i)     =solution((3*i)-1);      %Solution of vertical displacement       
        pore_pressure(i)        =solution(i*3);          %Solution of pore pressure
end
